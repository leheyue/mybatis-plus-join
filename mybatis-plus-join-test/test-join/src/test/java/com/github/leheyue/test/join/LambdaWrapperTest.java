package com.github.leheyue.test.join;

import com.baomidou.mybatisplus.core.conditions.query.LambdaQueryWrapper;
import com.baomidou.mybatisplus.core.metadata.IPage;
import com.baomidou.mybatisplus.core.toolkit.StringUtils;
import com.baomidou.mybatisplus.extension.plugins.pagination.Page;
import com.github.leheyue.test.join.dto.AddressDTO;
import com.github.leheyue.test.join.dto.UserDTO;
import com.github.leheyue.test.join.entity.*;
import com.github.leheyue.test.join.mapper.AddressMapper;
import com.github.leheyue.test.join.mapper.OrderMapper;
import com.github.leheyue.test.join.mapper.UserDTOMapper;
import com.github.leheyue.test.join.mapper.UserMapper;
import com.github.leheyue.test.util.Reset;
import com.github.leheyue.test.util.ThreadLocalUtils;
import com.github.leheyue.toolkit.JoinWrappers;
...
//                .selectCollection(UserDO.class, UserDO::getChildren)
    ...
    @Test
    void fromSub() {
        ThreadLocalUtils.set("SELECT t.id,t.user_id FROM ( SELECT case when t.id = 1 then 1 when t.id = 2 then 2 else 3 end AS create_by,t1.id AS user_id,t.id,t.`name` AS userName FROM `user` t LEFT JOIN address t1 ON (t1.user_id = t.id) WHERE (t.id <= ?) ) t LEFT JOIN address t1 ON (t1.id = t.id) WHERE (t.create_by <= ?)");
        MPJLambdaWrapper<UserDO> wrapper = JoinWrappers.lambda(UserDO.class)
